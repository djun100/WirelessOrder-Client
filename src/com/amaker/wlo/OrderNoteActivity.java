...
import com.amaker.wlo.provider.NoteAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.order_note);
		 noteDb = new NoteAdapter(OrderNoteActivity.this);
	        ...
	        addNote.setIndicator("添加备忘录", getResources().getDrawable(R.drawable.note_add));
	        addNote.setContent(R.id.note_add_layout);
	        tabHost.addTab(addNote);
	        
	        tabHost = getTabHost();
	        TabSpec queryNote = tabHost.newTabSpec(QUERY_NOTE);
	        queryNote.setIndicator("查询备忘录", getResources().getDrawable(R.drawable.note_query));
	 ...
	 private void setListenerObject() {
		 ...
		 noteAddSubmit.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if("".equals(content)){
					builder.setTitle("内容为空").setMessage("备忘录内容为空，请输入内容").setIcon(R.drawable.alert_wanring)
						.setCancelable(true).setPositiveButton("确定", new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog, int which) {}
						}).show();
				...
				noteDb.closeDB();
				if(r.intValue()!=-1){
					builder.setTitle("添加成功").setMessage("添加成功，您可以查看备忘录了！").setIcon(R.drawable.alert_add)
					.setCancelable(true).setPositiveButton("确定", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface dialog, int which) {
...
//							startActivity(i);
					...
					builder.setTitle("错误").setMessage("发生错误，备忘录添加失败！").setIcon(R.drawable.alert_error)
					.setCancelable(true).setPositiveButton("确定", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface dialog, int which) {}
					}).show();
