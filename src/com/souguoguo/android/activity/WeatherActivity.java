...
import com.souguoguo.android.util.CurrentCondition;
import com.souguoguo.android.util.ForecastCondition;
import com.souguoguo.android.util.OrderStringUtil;
import com.souguoguo.android.util.OrderUrlUtil;
import com.souguoguo.android.util.OrderWeatherHandler;
...
public class WeatherActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,
				android.R.layout.simple_spinner_item, OrderStringUtil.city);
		adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		weatherSpinner.setAdapter(adapter);
		weatherQuery.setOnClickListener(new Button.OnClickListener() {
			public void onClick(View v) {

				proDlg = OrderStringUtil.createProgressDialog(
						WeatherActivity.this,
						getResources().getString(R.string.pro_title),
						getResources().getString(R.string.pro_message), true,
						true);
				proDlg.show();
				...
				new Thread() {
					...
					public void run() {
						...
						try {
							URL url = new URL(OrderUrlUtil.WEATHER_URL
									+ cityCode);
	...
	private Handler handler = new Handler() {
		...
		public void dispatchMessage(Message msg) {
			...
			case OrderStringUtil.OK:
				...
				String baseUrl = OrderUrlUtil.GOOGLE_URL.substring(0,
						OrderUrlUtil.GOOGLE_URL.length() - 1);
				weatherView.setImageBitmap(OrderStringUtil
						.getBitMapForStringURL(baseUrl + cc.getIcon()));
				weatherText.setText(cc.toString());

				List<ForecastCondition> list = owh.getForecastList();

				ForecastCondition f = list.get(0);
				img01.setImageBitmap(OrderStringUtil
						.getBitMapForStringURL(baseUrl + f.getIcon()));
				text01.setText(f.toString());
				f = list.get(1);
				img02.setImageBitmap(OrderStringUtil
						.getBitMapForStringURL(baseUrl + f.getIcon()));
				text02.setText(f.toString());
				f = list.get(2);
				img03.setImageBitmap(OrderStringUtil
						.getBitMapForStringURL(baseUrl + f.getIcon()));
				text03.setText(f.toString());
				f = list.get(3);
				img04.setImageBitmap(OrderStringUtil
						.getBitMapForStringURL(baseUrl + f.getIcon()));
